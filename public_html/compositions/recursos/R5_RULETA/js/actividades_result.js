...
function filling_blanks_santiago_submit(evt) {
    ...
    retroalimentacion(strRetro);
    save_extra_data(objEvt, evt);
    upload_interaction(evt.json.preguntas, evt.answer, evt.results, evt.interactionType, evt);
    send_evt_to(evt.identify, objEvt, evt.results);
}
...
function check_answers(evt) {
    console.log("XXXXXXXXXXXXXXXXXXXXXXX CHECK ANSWERS XXXXXXXXXXXXXXXXXXXXXXX");
    console.log(evt.answer);
    console.log(!isEmpty(evt.answer));
    var is_empty = false;
...
function upload_interaction(json_data, answers, estado_answers, typeInteraction, evt) {
    ...
    if (!isEmpty(pagina.interaction)) {
        if (pagina.interaction.ALL) {
            var estado;
            console.log("aslkdaskldskjdjalkjdskldjalskdjajda"+ estado_answers);
            if (typeof estado_answers !== "string") {
                ...
                console.log(estado, estado_answers);
            } else {
                estado = estado_answers;
            }
            console.log("aslkdaskldskjdjalkjdskldjalskdjajda"+ estado);
