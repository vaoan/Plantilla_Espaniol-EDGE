...
EDGE_Plantilla.btn_inicio = ["btn_inicio_scrabble"];
...
$("body").on("EDGE_Recurso_PaginaOnShow", function (evt) {
    ...
    var sym_contenedor = buscar_sym(EDGE_Plantilla.plantilla_sym, EDGE_Plantilla.config.paginas[EDGE_Plantilla.pagina_actual].sym);
    ...
    $('iframe', sym_contenedor.ele)[0].contentWindow.$('body').trigger({
        type: "EDGE_Recurso_startTimerRemoto",
        sym: sym_modify
    });
});

$(document).on("TimeOut", function (evt) {
    var stage = $(EDGE_Plantilla.plantilla_sym.getComposition().getStage().ele);
    var stage2 = $(evt.sym.getComposition().getStage().ele);
    var identify = stage.prop("ed_identify");
    var result = check_every_answer();

    var timer = {};
    var timerObj = buscar_sym(evt.sym, stage2.prop("timer"), true);
